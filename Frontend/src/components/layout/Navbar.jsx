...
import { Menu, Search as SearchIcon, Bell, ChevronDown, ChevronRight, User } from 'lucide-react';
...
function getPageName(pathname) {
  ...
  const base = Object.keys(PAGE_NAMES).find(p => pathname === p || pathname.startsWith(`${p}/`));
    ...
    <header className="h-16 bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-[#2d1515]/60 sticky top-0 z-40 flex items-center justify-between px-4 gap-4">
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="hidden sm:block">
          <Link to="/dashboard" className="hover:opacity-90 transition-opacity">
            <AppLogo size="sm" showText showTagline={false} />
          </Link>
        </div>
        <nav className="hidden md:flex items-center gap-1.5 font-breadcrumb ml-2">
          <Link to="/dashboard" className="text-red-400/30 text-[12px] hover:text-red-400/60 transition-colors">Dashboard</Link>
          <ChevronRight className="w-3 h-3 text-red-800/40" />
          <span className="text-red-300/60 text-[12px] font-medium">{currentPage}</span>
        </nav>
      </div>
      ...
      <div className="flex items-center gap-3 shrink-0">
        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#1c1112] border border-[#2d1515]">
          <span className={`w-2 h-2 rounded-full ${isLive ? 'bg-green-500 animate-live' : 'bg-red-500'}`} />
        ...
        <div className="relative">
          ...
          >
            ...
            <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-red-600 rounded-full border border-[#0a0a0a]" />
          </button>
        ...
        <div className="relative ml-1">
          ...
          >
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-red-600 to-red-800 text-white flex items-center justify-center">
              <span className="font-logo text-[11px]">{getInitials(user?.name)}</span>
            </div>
            <ChevronDown className="w-4 h-4 text-red-400/60" />
          </button>

          {showUserMenu && (
            <div className="absolute right-0 mt-2 w-56 bg-[#1c1112]/95 border border-red-900/40 rounded-xl shadow-xl py-1 z-50 animate-scale-in backdrop-blur-md">
              <div className="px-4 py-3 border-b border-[#2d1515]">
                <p className="font-body font-semibold text-[14px] text-white tracking-[-0.01em]">
                  {user?.name || 'User'}
                </p>
                <p className="font-body-xs text-[11px] text-red-400/60 truncate mt-0.5">
                  {user?.email || '—'}
                </p>
              ...
              <div className="py-1">
                <Link
                  ...
                  className="w-full flex items-center gap-2 px-4 py-2 font-nav text-[13px] text-red-200/80 hover:bg-red-950/40 transition-colors"
                >
